...
mod_r<-cv.glmnet(x_train,y_train,alpha=0,
               lambda = grid, 
               nfolds = 5,
               standaradize = FALSE
)
...
plot(mod_r)
######## best lamda
(bestlam1<-mod_r$lambda.min)
######## which one is the best in the grid
which(mod_r$lambda.min==grid)
######## check whether the corresponding CV error is the smallest among all
mod_r$cvm[which(mod_r$lambda.min==grid)]==min(mod_r$cvm)

######## Refit ridge regression model on the full data set using lambda chosen from cross-validation
mod<-glmnet(house_X,house_Y,alpha=0)
predict(mod,type="coefficients",s=bestlam1)[1:22,]
...
abline(0, 0) 
pred <- predict(mod,s=bestlam1,newx=x_test)
...
########### Make predictions 
predictions_test <- predict(mod,s=bestlam1,newx=x_test)
predictions_train <- predict(mod,s=bestlam1,newx=x_train)
